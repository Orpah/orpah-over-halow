...
场景（全部确定性）：
    ...
    C 同数据只看前 2min（+200ppm）    → 漂移「基线不足」——不编数，也不硬说成噪声
...
def obs(n, dt=1.0, drift_ppm=0.0, offset=0.0, seed=1, t_off=0.0):
    ...
    模型说明（免得再来回猜）：`dev = int(rx + offset + r·(rx − t0))` 就是
    “**偏移 = offset + r·真实经过时间**”，即 `offset(t)` 线性于**真实（服务器）时间** —— 精确，不是近似。
    换个写法“设备时钟从 t0 起以 `1+r` 的速率走”得到的式子
    `(t0+offset) + (1+r)·(rx−t0)` 与之**代数恒等**（展开即 `rx + offset + r·(rx−t0)`）；
    实测 20 万条样本两种写法给出**完全相同**的 `dev`（`._tmp` 探针已删，结论记在这里）。
    `int()` 截断才是唯一的“额外效应”，它就是文档里反复提的 1s 量化噪声。
    """
