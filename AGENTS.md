...
- **回放页硬规则（2026-09-12）**：
  ...
  - **告警通知（2026-09-13，§三）**：告警是**状态**（`/api/alerts` 每 3 s 重算，页面红点），
    ...
    - **阈值必须实测定，不许拍脑袋**：`rssi_jump` 的 25 dB 来自「诚实序列 12956 对样本」（四种上报间隔 × 2 h）
      的零假设分布（max 12 dB），且只在 **0.5~15 s** 的相邻样本间比（跳变随间隔线性增长）；
      `test_alerts.py` 有**零误报回归锁**。两条新规则都属**线索型**：文案必须带「仅线索、需人工复核」。
