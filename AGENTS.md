...
## 0d. 文档/README：图文并茂（2026-09-21 用户定，适用于所有项目）

- **本仓每个 README（顶层 + 子目录）都要把「这样的图」嵌进正文**：接线图 / 原理框图 / **界面截图** /
  实测曲线 —— 用户原话「**图对人的阅读吸引力更大**」；**别只给文字表格、也别只放一个链接**。
- 写法：markdown 相对路径直接嵌（`[![caption](x.svg)](x.svg)`；`.svg` / `.png` 都能渲染），
  图旁配一句 caption。
- **图要与文字/规格一致、不能是画饼**：界面截图就截真实页面的（`ui_server.py` 起页后用浏览器截，
  别手绘 UI）；数据曲线注明「演示数据 / 仿真数据，不是实测」。
- 本仓现况：顶层 `README.md` 455 行**一图没有**（待补：拓扑、告警、可信度/偏差扫描、回放与能量）；
  文案不得写 Markdown（§0）那条同样适用于 caption。
