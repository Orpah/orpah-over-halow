...
| `rssi_jump` | `warn` | 设备**自报**的链路强度相邻样本跳变超阈（**仅线索、需人工复核**）；**只在间隔 0.5~450 s 的相邻样本间比**（2026-09-14 按设计常态重标定：450 = 1.5 × 最长合法上报间隔 300 s → **覆盖常态 60 s**；再长就分不清“突变”与“设备本来就报得慢”→ 不比，宁可不报） | `25` dB | `ORPAH_ALERT_RSSI_JUMP_DB` |

⚠ **两条新规则（2026-09-13）为什么敢用这个阈值**：按**实测零假设分布**定，不拍脑袋。
诚实序列 = 演示行走模型里设备自报的 rssi（含 ±2 dBm 确定性噪声）按各间隔采样，量相邻样本 `|Δ|`。
**2026-09-14 按设计常态重标定**（dt 1 s~1800 s、**168 h、816803 对**）：**max 19 dB、p99.9 15 dB、
>20 dB 的 0 对**；且发现**跳变只在 1→30 s 增长、之后饱和**（序列是“最近那台路由器测到的强度”，
几何自归一）—— 旧注释写的“随间隔线性增长”只在那一小段成立。⇒ **窗口**改为 0.5~**450 s**
（覆盖常态 60 s 与降级 300 s），**阈值仍 25 dB**（诚实上界 19 dB 之上仍有 1.3× 余量）。
回归锁：`test_alerts.py` 用 1/2/5/10 s×2h + 15/30/60/90/300/450 s×168h 诚实序列断言**零误报**
...
⚠ **2026-09-14 已收口**：`rssi_jump` 的窗口按设计常态重标定为 0.5~450 s（阈值 25 dB 不变，实测上界 19 dB、零误报）
— 详见上一条；**不再存在“常态下它不触发”这个问题**。
...
**报文流时间轴（页面行为，2026-09-13，`replay.html` §3）**：回放页用 `points` 的 `seq` 画一条
「原始上报帧」时间轴（**没有新接口**）—— 判定走 `pos.js` 的 `frameGaps()`（**单一实现 + 单测**）：

| 相邻两帧的 `seq` 差 | 判定 | 含义 |
|---|---|---|
| `d > 1` | 缺口 `d-1` | **上行丢包证据** |
| `d === 0` | 重复 | 同序号重复到达 |
| `d < 0` | **回退**（不是缺口） | 设备重启 / 乱序（实测演示数据里就是“重启 UI 服务器”造成的） |
| 任一帧 `seq` 为 `null`/`undefined`/`""` | **不判** | 没值不猜（`Number(null)===0` 会让整段变成“一路回退”，已加守卫 + 单测） |
