...
| **客户端设备仿真器（真机三步走的第 a 步）**：`client_sim.py` 的 `DeviceSim` —— 把"一台 ORPAH 客户端设备"按规范跑起来（会话节奏 / `cap.rtc` / 无 RTC `ts=0` / §8.2 故障选级 / 电量 / 能量 / 自愿自限频），**不重写协议**（报文/签名/选级/限频/间隔全部来自各自单一源，`test_client_sim.py` 有源码守卫）。**接真板只换底层传输**（`DeviceSim(client=…)`）：b 步走 **UART**（`--transport serial`，`host_serial.SerialAtBus`：`AT+TXDATA` 上行 / `FRAME:RX` 下行），**真机未验证**（上机首测事项见 `docs/client_sim.md` §2.3）。验收：`test_client_sim.py`（离线 37 项）+ `demo_client_sim.py`（端到端：验签通过 / 下行到达 / 零丢弃）+ `demo_client_uart.py`（UART 线路协议排练） | `client_sim.py` + `host_serial.py` + `client.py`（`ClientHost`）+ `orpah_id`/`orpah_proto`/`energy`/`ratelimit` | `test_client_sim.py` + `demo_client_sim.py` + `demo_client_uart.py` + `docs/client_sim.md` |
...
| `replay.html` 回放 | 时间窗回放、逐帧定位、平滑、有效时段分色、事件时间线、**报文流时间轴（序号缺口/回退/帧间隔）**、轨迹导出 GPX/GeoJSON/**CSV**（含**真值对照**）、**误差 CDF（仅模拟环境）**、**基点与实时页共享同一份** | `/api/replay`、`/api/stations`、`/api/registry`、`/api/config`、`/api/truth` | **只读** IoTDB |
...
> 回放那张要 IoTDB（默认 `127.0.0.1:6667`）；没起时会**自动跳过并打印原因**，不拿空图充数。
...
回放（`replay.html`，读 IoTDB 时间窗）：绿线=真实轨迹（仅模拟环境有真值）、虚线=估计轨迹、
红叉=当前估计点、黄虚线椭圆=95% 置信范围、蓝方块=四台路由器（含距离环）——
**回放只看光标之前的样本**（不“看到未来”），所以这张是先点「▶ 播放」让光标走了一段再截的：

[![回放：真实轨迹 vs 估计轨迹与 95% 置信椭圆](docs/images/ui-replay.png)](docs/images/ui-replay.png)
...
- **人级聚合（演示，`定位与轨迹` 页真实模式）**：一个人的多台客户端（小明：项链 + 鞋）各自定位 →
  1/σ² 加权合并成**一个人**的位置。⚠ 本 demo 只有**一台**真的在周期上报，
  **同名下其它设备的观测是按同一个 motion 位置模型生成的模拟数据**（噪声按 `<sid>|<sn>` 独立），
  真机是各设备自己上报；页面与 README 都明说，不包装成实测。
...
```bash
python run_checks.py            # 26 个离线套件（各模块自检 + 批量合规 + 抓包解析 + 时钟漂移 + 能量轴/标定 + pos.js 内核 + 文案字典 + UI 窄屏/信息层级/数据新鲜度守卫），约 8 秒
