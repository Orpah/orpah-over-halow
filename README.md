...
```bash
cd simulator/orpah
python run_checks.py            # 11 个离线套件（各模块自检 + 批量合规 + 抓包解析），约 2 秒
...
└── orpah/
    ...
    ├── run_checks.py     # 【测试台】11 个离线套件一键跑 + 出报告（--e2e 再加 5 个 demo）
...
7. **一键回归**：`python run_checks.py`（11 个离线套件，~2s）→ `checks_report.md`；
