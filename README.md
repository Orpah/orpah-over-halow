...
| **客户端设备仿真器（真机三步走的第 a 步）**：`client_sim.py` 的 `DeviceSim` —— 把"一台 ORPAH 客户端设备"按规范跑起来（会话节奏 / `cap.rtc` / 无 RTC `ts=0` / §8.2 故障选级 / 电量 / 能量 / 自愿自限频），**不重写协议**（报文/签名/选级/限频/间隔全部来自各自单一源，`test_client_sim.py` 有源码守卫）。**接真板时只换底层传输**（`DeviceSim(client=…)` 一个参数）；真板传输**未实现**（等物理通路定，见 `docs/client_sim.md` §2.3）。验收：`test_client_sim.py`（离线 28 项）+ `demo_client_sim.py`（端到端：验签通过 / 下行到达 / 零丢弃） | `client_sim.py` + `client.py`（`ClientHost`）+ `orpah_id`/`orpah_proto`/`energy`/`ratelimit` | `test_client_sim.py` + `demo_client_sim.py` + `docs/client_sim.md` |
