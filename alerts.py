...
- 阈值默认值是**演示压缩时间**（真实部署要调大），但**全部可用环境变量覆盖**
  ...
  **随节拍走**，而不随演示快慢走（以前写 30 s 是拿 2 s 演示周期拍的）。
  同上理由（2026-09-14 再修）：`rssi_jump` 的**采样窗口**也从“2 s 演示周期的 0.5~15 s”
  改成 **0.5~450 s**（= 1.5 × 最长合法上报间隔）—— 否则常态 60 s 下它**根本不会触发**
  （那是错的，见下「RSSI 突变阈值与窗口怎么定的」：重测后**阈值不用改**，25 dB 依旧零误报）。
...
**「RSSI 突变」阈值与窗口怎么定的（2026-09-14 按设计常态重新标定，不拍脑袋）**：
诚实序列 = 演示行走模型里设备**自报**的 rssi（`motion.Walk.nearest()`，含 ±2 dBm 确定性噪声；
**取“当前关联的那台”的测量**），按各种上报间隔取样，量**相邻样本 |Δ|**：

    间隔      1s    5s   15s   30s   60s   90s  120s  300s  450s 1800s
    中位      1.0   2.0   5.0   3.0   4.0   4.0   5.0   3.0   4.0   3.0
    p99       4.0   7.0  12.0  16.0  14.0  13.0  12.0  16.0  13.0  15.0
    max       6.0  10.0  14.0  19.0  17.0  15.0  14.0  18.0  16.0  16.0   （168 h）

★ **旧注释说“跳变随采样间隔线性增长”，只在 1→30 s 那一小段成立** —— 过了约 30 s 就**饱和**
（实测 168 h：30 s 的 19 dB 已是全局最大，再长反而略降）。原因是这个序列是“**离我最近那台的**
链路强度”：人走远了就会换一台，而总有一台在附近 → 
|Δ| 被几何自归一化，不会随 dt 无限长。
⇒ 实测**全局**（dt 1 s~1800 s、168 h、**816803 对样本**）：**max 19 dB、p99.9 15 dB、
超过 20 dB 的 0 对**。所以：

- **窗口**（2026-09-14）：`RSSI_PAIR_MIN_SEC`~`RSSI_PAIR_MAX_SEC` = **0.5 s ~ 450 s**
  （= 1.5 × `energy.MAX_USEFUL_INTERVAL_S`）—— 覆盖设计常态（60 s）与**降级/紧急上报**
  （最长合法间隔 300 s）+ 抖动。**旧值 15 s 是在看 2 s 演示周期时拍的**，那时还不知道
  常态是 60 s → 改成常态基准后**它就不再触发了**（那是错的，已修）。
- **阈值仍 25 dB**：对**新窗口、更长样本**依旧是 **0 误报**（816803 对全 dt、一周）——
  诚实上界 19 dB 之上还有 1.3× 余量（**不再声称 2×**：那是个笼统的说法，这里是实测量）。
  不改这个数的理由：这阈值本来就是一条**线索**，抬到 40 dB 反而会把 30 dB 量级的
  “天线松了/被遮挡/设备被动手脚”全放过 —— 而那正是它存在的意义。
- **仍然只是线索**：它只看“相邻两次自报之间的跳变”，**判不了原因**（走动、遮挡/多径、天线/接头、
  标定漂移，也包括被改装），文案必须带「仅线索、需人工复核」。
- 回归锁：`test_alerts.py` 用**诚实序列在 1/5/15/60/300/450 s 上零误报**（站位取
  `ui_server.DEMO_STATIONS` 单一源，几何改了锁会红）。
...
# RSSI 突变（2026-09-13 加，2026-09-14 按设计常态重标定）：**只在相邻样本间隔 0.5~450 s 时比**。
# 450 s = 1.5 × `energy.MAX_USEFUL_INTERVAL_S`（设备最长合法上报间隔 300 s）+ 抖动 ——
# 常态 60 s 与降级上报都落在窗口里，而**再长就没法归因**（分不清“设备报得慢”与“不在这一带”）。
# 阈值 25 dB 实测对**全 dt（1 s~1800 s）、168 h、816803 对**诚实样本零误报（上界 19 dB）。
RSSI_JUMP_DB = _env_int("ORPAH_ALERT_RSSI_JUMP_DB", 25)
RSSI_PAIR_MIN_SEC = 0.5
RSSI_PAIR_MAX_SEC = int(1.5 * en.MAX_USEFUL_INTERVAL_S)
...
def evaluate(registry, cases, id_reports, now=None, clock=None, energy=None,
    ...
    #    **只比 0.5~450 s 的相邻样本**（2026-09-14 重标定）：窗口按**最长合法上报间隔**定，
    #    覆盖常态 60 s 与降级上报；再长就没法归因（分不清「突变」与「设备本来就报得慢」）→ 不比。
    #    （旧注释曾写“跳变随间隔线性增长”→ 实测只在 1→30 s 成立，之后饱和，见模块头。）
