...
/* 本地坐标（米，x 向东 / y 向北）→ [纬度, 经度]。
   ...
   两个常数的来历与精度（**显示/导出用，不影响本地米制坐标**）：
   · 纬度 110540 m/°：常用的“每度纬度米数”近似值（真实值随纬度变：赤道 110574、
     40°N 约 111035、极点 111694）→ 本 demo 场景（31~40°N）在**经纬度输出**上偏约 0.4%
     （1 km 南北向约差 4 m）。定位、半径、椭圆、站位坐标都算在**本地米制**里，不受它影响；
   · 经度 111320·cos(基点纬度)：等距圆柱近似，基点离场地越远误差越大（见 UI 提示：基点要取在场地方圆几公里内）。
   要更准就换 WGS84 子午线弧长公式 —— 但那会让“演示用的几公里场景”看不出差别，先不做。 */
