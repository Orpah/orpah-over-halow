...
| UI 样式/窄屏守卫 | `test_uicss.py` | ✅ PASS | 2.9s | 通过 |
...
### 能量轴（免电池客户端） — PASS（0.0s）
    ...
    == 6. 完全没采集：**如实沉默**，不编间隔 ==
      OK   不建模监听时 listen_interval_s=None（**不撒谎说 0s**）
    能量轴测试全部通过
...
### 防 spoof 空口端到端 — PASS（1.7s）
      ...
      攻击者 SN    : CN-WH01-4X00CKRZBQ-25（未登记 → 服务器不认识）
